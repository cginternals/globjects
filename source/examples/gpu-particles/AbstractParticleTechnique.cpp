...
void AbstractParticleTechnique::initialize(const std::string & vertexShaderSourceFilePath)
{
    ...
    m_color = new globjects::Texture(GL_TEXTURE_2D);
    ...
    m_fbo->unbind(GL_FRAMEBUFFER);


    m_clear = new ScreenAlignedQuad(
        globjects::Shader::fromFile(GL_FRAGMENT_SHADER, "data/gpu-particles/clear.frag"));
    m_quad = new ScreenAlignedQuad(m_color);

    m_drawProgram = new globjects::Program();
    m_drawProgram->attach(
        globjects::Shader::fromFile(GL_VERTEX_SHADER, vertexShaderSourceFilePath)
      , globjects::Shader::fromFile(GL_GEOMETRY_SHADER, "data/gpu-particles/points.geom")
      , globjects::Shader::fromFile(GL_FRAGMENT_SHADER, "data/gpu-particles/points.frag"));
