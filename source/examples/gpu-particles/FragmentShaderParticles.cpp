...
void FragmentShaderParticles::initialize()
{
    ...
    m_quadUpdate = new ScreenAlignedQuad(
        createShaderFromFile(gl::FRAGMENT_SHADER, "data/gpu-particles/particle.frag"),
        m_texPositions );
    ...
    m_clear = new ScreenAlignedQuad(
        createShaderFromFile(gl::FRAGMENT_SHADER, "data/gpu-particles/clear.frag") );
    m_quad  = new ScreenAlignedQuad(m_colorBuffer);

    // Create draw program
    m_drawProgram = new Program();
    m_drawProgram->attach(
        createShaderFromFile(gl::VERTEX_SHADER,   "data/gpu-particles/points_fragment.vert")
    ,   createShaderFromFile(gl::GEOMETRY_SHADER, "data/gpu-particles/points.geom")
    ,   createShaderFromFile(gl::FRAGMENT_SHADER, "data/gpu-particles/points.frag"));
}
