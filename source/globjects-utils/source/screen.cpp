#include <globjects-utils/screen.h>
...
,   unsigned int & height)
{
    ...
    if (0 == dc)
    {
        glo::warning() << "Could not't get a device context from the desktop window.";
        return;
    }
    ...
    if(0 == ReleaseDC(hWnd, dc))
        glo::warning() << "Releasing temporary (query) device context failed.";
