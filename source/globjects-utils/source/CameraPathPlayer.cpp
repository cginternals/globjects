...
#include <globjects-utils/math/BezierCurve.h>
...
CameraPathPoint CameraPathPlayer::interpolate(const PathSection& section, const float t)
{
    ...
    //GLOBJECTS_BASE::debug() << eye << " (" << (p1.eye*(1.0f-t) + p2.eye*t) << ")";
...
void CameraPathPlayer::createVao()
{
    ...
    StringTemplate* vertexShaderSource = new gloutils::StringTemplate(new glo::StaticStringSource(vertexSource));
    StringTemplate* fragmentShaderSource = new gloutils::StringTemplate(new glo::StaticStringSource(fragmentSource));
    ...
    m_program->addUniform(new glo::Uniform<mat4>("transform"));
}
